...
const char codebouton[]="1111111111"; //  code à  envoyer
const char codeultrason[]="0110110110"; //  code à  envoyer
const char codefourche[]="1100110011"; //  code à  envoyer
...
const int tempsHaut1init=15; //durée ms niveau haut pour un 1 sur les 3 premiers et derniers bits
const int tempsBas1init=5; //durée ms niveau bas pour un 1 sur les 3 premiers et derniers bits
const int tempsHaut0=3; // durée ms niveau haut pour un 0
const int tempsBas0=10; // durée ms niveau bas pour un 0
const int tempsHaut0init=5; //durée ms niveau haut pour un 0 sur les 3 premiers et derniers bits
const int tempsBas0init=15; //durée ms niveau bas pour un 0 sur les 3 premiers et derniers bits
...
void loop() {
  ...
  if(testDepartGarage == 1) {
    if(ledRouge==0) {
      ...
      angleModif(110); // ouverture garage
      digitalWrite(pinLedRouge,LOW);
      testDepartGarage=0;
      Emission(codefourche);
      delay(5000); //a supprimer si code incorrect
      testfourche:
      if(testDepartGarage==0) { //a supprimer si code incorrect
        digitalWrite(pinLedRouge,HIGH); //a supprimer si code incorrect
        angleModif(20); // fermeture garage //a supprimer si code incorrect
        digitalWrite(pinLedRouge,LOW); //a supprimer si code incorrect
        testDepartGarage=0; //a supprimer si code incorrect
        } //a supprimer si code incorrect
        else{ //a supprimer si code incorrect
          goto testfourche; //a supprimer si code incorrect
          } //a supprimer si code incorrect
    }
//    else { //enlever le commentaire si code incorrect
//    delay(3000); //delai avant fermeture barrière //enlever le commentaire si code incorrect
//    digitalWrite(pinLedRouge,HIGH); //enlever le commentaire si code incorrect
//    angleModif(20); // fermeture garage //enlever le commentaire si code incorrect
//    digitalWrite(pinLedRouge,LOW); //enlever le commentaire si code incorrect
//    testDepartGarage=0; //enlever le commentaire si code incorrect
//    Emission(codefourche); //enlever le commentaire si code incorrect
//    } //enlever le commentaire si code incorrect
  }
...
void Emission(const char* code) 
{
  ...
  for(int j=0;j<nbrepet;j++) { // boucle pour nbrepet répétitions de l'émission
    digitalWrite(pinEmetteur,HIGH); //début d'émission des 3 premiers bits
    digitalWrite(pinLedBleue,HIGH);
    delay(tempsHaut1init); // niveau HIGH pendant tempsHaut1init ms
    digitalWrite(pinEmetteur,LOW);
    digitalWrite(pinLedBleue,LOW);
    delay(tempsBas1init); // niveau LOW pendant tempsBas1init ms
    digitalWrite(pinEmetteur,HIGH);
    digitalWrite(pinLedBleue,HIGH);
    delay(tempsHaut0init); // niveau HIGH pendant tempsHaut0init ms
    digitalWrite(pinEmetteur,LOW);
    digitalWrite(pinLedBleue,LOW);
    delay(tempsBas0init); // niveau LOW pendant tempsBas0init ms
    digitalWrite(pinEmetteur,HIGH);
    digitalWrite(pinLedBleue,HIGH);
    delay(tempsHaut0init); // niveau HIGH pendant tempsHaut0init ms
    digitalWrite(pinEmetteur,LOW);
    digitalWrite(pinLedBleue,LOW);
    delay(tempsBas0init); // niveau LOW pendant tempsBas0init ms
    //fin d'émissions des 3 premiers bits
    
    
    for(int i=0;i<10;i++) { // boucle sur les 10 digits du code (sans les 3 premiers et derniers)
      if(code[i]=='1') {
        // émission d'un "1"
        digitalWrite(pinEmetteur,HIGH);
        digitalWrite(pinLedBleue,HIGH);
    ...
    digitalWrite(pinEmetteur,HIGH); //début d'émission des 3 derniers bits
    digitalWrite(pinLedBleue,HIGH);
    delay(tempsHaut0init); // niveau HIGH pendant tempsHaut0init ms
    digitalWrite(pinEmetteur,LOW);
    digitalWrite(pinLedBleue,LOW);
    ...
    digitalWrite(pinLedBleue,LOW);
    delay(tempsBas1init); // niveau LOW pendant tempsBas1init ms
    //fin d'émission des 3 derniers bits
